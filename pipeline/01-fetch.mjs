...
const PPIO_KEYWORDS = {
  critical: [
    ...
    /国常会.*数字中国/, /国常会.*新兴产业/, /数字中国.*算力/, /数字中国.*数字基础设施/,
    ...
    /NVDB.*通报/, /NVDB.*风险提示/, /NVDB.*AI/, /工信部.*网络安全.*平台.*AI/,
    /Claude Code.*后门/, /Claude Code.*安全/, /AI编程工具.*后门/, /AI编程工具.*安全.*隐患/,
    /闭源.*后门/, /闭源模型.*安全/, /闭源.*监控/, /闭源.*数据回传/,
    /AI工具.*监控.*回传/, /AI工具.*安全后门/, /安全后门.*AI/, /后门.*回传.*敏感/,
...
function entitySignature(title) {
  ...
  const topics = [
    ...
    ['数字中国.*算力|数字基础设施.*超前|国常会.*数字中国|数智化.*算力网', '数字中国算力'],
    ['新兴支柱产业|新兴产业.*全链条|新兴产业.*规模化.*AI', '新兴支柱产业'],
...
function buildSearchQueries(config) {
  ...
  // 数字中国 + 新兴支柱产业 — 7/10国常会核心信号
  queries.push({ q: '数字中国 算力 数字基础设施 适度超前 2026', category: '政策', boost: 30 });
  queries.push({ q: '国常会 数字中国 新兴产业 数字基础设施 2026', category: '政策', boost: 30 });
  queries.push({ q: '新兴支柱产业 规模化 AI 算力 全链条 2026', category: '政策', boost: 25 });
  ...
  // NVDB/工信部漏洞平台 — 安全及闭源模型工具检测
  queries.push({ q: 'site:nvdb.org.cn AI 模型 安全 漏洞 2026', category: '监管', boost: 30 });
  queries.push({ q: '工信部 NVDB 网络安全 威胁 漏洞 AI 2026', category: '监管', boost: 25 });
  queries.push({ q: 'NVDB 风险提示 AI工具 安全后门 2026', category: '监管', boost: 25 });
  // 闭源模型/AI工具安全风险
  queries.push({ q: '闭源模型 安全风险 后门 监控 2026', category: '监管', boost: 25 });
  queries.push({ q: 'AI编程工具 安全 后门 回传 数据 2026', category: '监管', boost: 25 });
  queries.push({ q: 'Claude Code 安全 后门 工信部 通报 2026', category: '监管', boost: 30 });
  queries.push({ q: 'Anthropic Claude Code 安全风险 监控 2026', category: '监管', boost: 20 });
  queries.push({ q: '闭源 AI 工具 数据回传 合规 审查 2026', category: '监管', boost: 20 });
